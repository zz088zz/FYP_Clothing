...
<?php
	if(ISSET($_POST['cont'])){
		...
		$query = mysqli_query($connect, "SELECT * FROM `users` WHERE `id` = '$id' AND `password` = '$user_password'");
		$row = mysqli_num_rows($query);
		
		if($row > 0){
            $sql=mysqli_query($connect, "update users set password = '$newpass' where `id` = '$id'");
...
<div style="margin: auto; border:1px solid #DDD; border-radius:10px; width:400px; padding:0px; background-color: white; margin-top:80px" >
	...
	<div id="login-title">
		<h4 style="margin:0px; padding:12px 40px; color:blue; font-family:Arial;">Change Password</h4>
	</div>	
	<div id="login-form">
		<form name="loginfrm">
			<p><input type="password" name="user_password" placeholder="Enter your old password" minlength="8" maxlength="16" required/></p>
